          ...
          <form action="{{ route('motorista.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  type="text" id="cnh" name="cnh" required
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
              ...
              <div>
                ...
                <input
                  type="date" id="validade_cnh" name="validade_cnh" required
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="text" id="telefone" name="telefone" required
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  @foreach($motoristas as $motorista)
                    <tr class="hover:bg-gray-50" id="view-row-{{ $motorista->id }}">
                      ...
                      <td class="px-4 py-2 text-sm text-gray-700">{{ \Carbon\Carbon::parse($motorista->validade_cnh)->format('d/m/Y') }}</td>
                      <td class="px-4 py-2 text-sm text-gray-700">{{ \Carbon\Carbon::parse($motorista->data_nascimento)->format('d/m/Y') }}</td>
                      ...
                      <td class="px-4 py-2 text-sm text-gray-700 text-center space-x-2">
                        ...
                        <form action="{{ route('motorista.destroy',$motorista->id) }}" method="POST" class="inline">
                          ...
                          <button
                            ...
                            onclick="return confirm('Deseja excluir este motorista?')"
                          >Excluir</button>
                        </form>
                      </td>
                    </tr>
                    <tr id="edit-row-{{ $motorista->id }}" class="hidden bg-gray-50">
                      ...
                      <form action="{{ route('motorista.update',$motorista->id) }}" method="POST" class="w-full">
                        ...
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="text" name="nome" value="{{ $motorista->nome }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="text" name="cnh" value="{{ $motorista->cnh }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="date" name="validade_cnh" value="{{ $motorista->validade_cnh }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="date" name="data_nascimento" value="{{ $motorista->data_nascimento }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="text" name="endereco" value="{{ $motorista->endereco }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="text" name="telefone" value="{{ $motorista->telefone }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm">
                          <input
                            type="email" name="email" value="{{ $motorista->email }}"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          />
                        </td>
                        <td class="px-4 py-2 text-sm text-center space-x-2">
                          <button type="submit" class="inline-flex px-2 py-1 bg-green-600 text-white text-xs font-medium rounded hover:bg-green-700 transition">
                            Salvar
                          </button>
                          <button type="button" onclick="cancelEdit({{ $motorista->id }})" class="inline-flex px-2 py-1 bg-gray-200 text-gray-800 text-xs font-medium rounded hover:bg-gray-300 transition">
                            Cancelar
                          </button>
                        </td>
                      </form>
